 ...
 * @license  URI: http://www.gnu.org/licenses/gpl-2.0.html
 * @link     https://sms.softeriatech.com/
 * SOFTSMAL_WPLogin class.
 **/
class SOFTSMAL_WPLogin extends FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = SOFTSMAL_FormSessionVars::WP_LOGIN_REG_PHONE;
     ...
     * @var stirng
     */
    private $form_session_var2 = SOFTSMAL_FormSessionVars::WP_DEFAULT_LOGIN;
     ...
     * @var stirng
     */
    private $form_session_var3 = SOFTSMAL_FormSessionVars::WP_LOGIN_WITH_OTP;
    ...
    public function handleForm()
    {
        ...
        if ('on' === $enabled_login_with_otp ) {
            if (is_plugin_active('easy-login-woocommerce/xoo-el-main.php') || is_plugin_active('easy-login-woocommerce-premium/xoo-el-main.php') ) {
				add_action('xoo_el_form_end', array( $this, 'smsproDisplayLoginWithOtp' ), 10, 2);
				if (!has_action('xoo_el_form_end', array($this, 'smsproDisplayLoginWithOtp'))) {
                  add_action('xoo_el_login_form_end', array( $this, 'smsproDisplayLoginWithOtp' ));
				}
            }
            add_action('woocommerce_login_form_end', array( $this, 'smsproDisplayLoginWithOtp' ));
            add_action('um_after_login_fields', array( $this, 'smsproDisplayLoginWithOtp' ), 1002);
            add_action('thim_after_login_form', array( $this, 'smsproDisplayLoginWithOtp' ), 10);            
        }
    ...
    public function handleLoginPopup()
    {
        ...
        if (! SOFTSMAL_cURLOTP::validateCountryCode($phone_number)) {        
            return $data;
        }
        ...
        softeria_alerts_site_challenge_otp($username, null, null, $phone_number, 'phone', $password, SOFTSMAL_Utility::currentPageUrl(), true);
    }
    ...
    public function handleLoginWithOtp()
    {
        ...
        if (isset($_REQUEST['username']) ) {
            ...
            $billing_phone = SOFTSMAL_cURLOTP::checkPhoneNos($phone_number);
            if (SOFTSMAL_Utility::isBlank($phone_number)) {
            ...
            if (! empty($user_login) ) {
                ...
                wp_send_json(SOFTSMAL_Utility::_create_json_response(SOFTSMAL_Messages::showMessage('PHONE_NOT_FOUND'), 'error'));
            }
        }
    }
    ...
    public function loginWpUser( $user_log, $extra_data = null )
    {
        ...
        $redirect = SOFTSMAL_Utility::isBlank($extra_data) ? site_url() : $extra_data;
    ...
    {
        if (! $this->checkWpLoginByPhoneNumber() || ! SOFTSMAL_Utility::validatePhoneNumber($username) ) {
            return $user;
        }
    ...
    public static function getUserFromPhoneNumber( $username, $key )
    {
        ...
        $wcc_ph     = SOFTSMAL_cURLOTP::checkPhoneNos($username);
        $wocc_ph    = SOFTSMAL_cURLOTP::checkPhoneNos($username, false);
        $wth_pls_ph = '+' . $wcc_ph;
    ...
    public function askPhoneAndStartVerification( $user, $key, $username, $phone_number )
    {
        ...
        if (! $this->checkWpLoginRegisterPhone() ) {
            softeria_alerts_site_otp_validation_form(null, null, null, SOFTSMAL_Messages::showMessage('PHONE_NOT_FOUND'), null, null);
        } else {
            SOFTSMAL_Utility::initialize_transaction($this->form_session_var);
            softeria_alerts_external_phone_validation_form(SOFTSMAL_Utility::currentPageUrl(), $user->data->user_login, __('A new security system has been enabled for you. Please register your phone to continue.', 'soft-sms-alerts'), $key, array( 'user_login' => $username ));
        }
    }
    ...
    public function fetchPhoneAndStartVerification( $user, $key, $username, $password, $phone_number )
    {
        ...
        softeria_alerts_site_challenge_otp($username, null, null, $phone_number, 'phone', $password, SOFTSMAL_Utility::currentPageUrl(), false);
    }
    ...
    public function handleWpLoginAjaxSendOtp( $data )
    {
        ...
        if (! $this->checkWpLoginRestrictDuplicates()
            && ! SOFTSMAL_Utility::isBlank($this->getUserFromPhoneNumber($data['billing_phone'], $this->phone_number_key)) 
        ) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response(__('Phone Number is already in use. Please use another number.', 'soft-sms-alerts'), SOFTSMAL_Constants::ERROR_JSON_TYPE));
    ...
    public function handle_failed_verification( $user_login, $user_email, $phone_number )
    {
        ...
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            softeria_alerts_site_otp_validation_form($user_login, $user_email, $phone_number, SOFTSMAL_Messages::showMessage('INVALID_OTP'), 'phone', false);
        }
