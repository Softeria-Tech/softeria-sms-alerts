 ...
 * @link     https://sms.softeriatech.com/
 *
 * class SOFTSMAL_Elementor
 */  
class SOFTSMAL_Elementor extends FormInterface
{
    /**
     * Elementor form key
     *
     ...
     */
    private $form_session_var = SOFTSMAL_FormSessionVars::ELEMENTOR_FORM;
    ...
    public function elementorFormValidationErrors( $record, $ajax_handler )
    {
        ...
        if (isset($user_phone) && SOFTSMAL_Utility::isBlank($user_phone) ) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response(__('Please enter phone number.', 'soft-sms-alerts'), SOFTSMAL_Constants::ERROR_JSON_TYPE));
            exit();
        }
    ...
    public function processFormFields( $user_phone )
    {
        ...
        if (! isset($phone_num) || ! SOFTSMAL_Utility::validatePhoneNumber($phone_num) ) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response(str_replace('##phone##', $getdata['user_phone'], $phoneLogic->_get_otp_invalid_format_message()), SOFTSMAL_Constants::ERROR_JSON_TYPE));
            exit();
        }
...
class Elementor extends ElementorPro\Modules\Forms\Fields\Field_Base
{
    ...
    {
        return __('SOFTSMSAlerts', 'soft-sms-alerts');
    }
    ...
    public function checkSmsalertField($obj, $datas)
    {
        if (!empty($datas['elements'])) {
                        ...
                        if (array_key_exists('elements', $element) ) {
                            foreach ( $element['elements'] as $setting ) {
                                if (array_key_exists('settings', $setting) ) {
                                    if (!empty($setting['settings']['submit_actions']) && in_array("softsmsalerts", $setting['settings']['submit_actions']) ) {
            ...
            if ($softeria_alerts_action_added && !$softeria_alerts_field_added) {
                wp_send_json_error([ 'statusText' => esc_html__('Please add field type Softeria Tech in your form.', 'soft-sms-alerts'),'readyState'=>4,'status'=>500 ]);
            }
        }
    }
                    ...
                    if ('true' === $settings['otp_verification_enable'] ) {
                        ...
                        $inline_script .= 'jQuery(document).ready(function(){
                            function addModalInForm(){
                                jQuery(".modal.smsproModal").each(function(){

                                    var form_id = jQuery(this).attr("data-form-id");

                                    if ( form_id.indexOf("saFormNo_") > -1){

                                        var class_unq = form_id.substring(form_id.indexOf("_")+ 1);                                jQuery("#sa_verify_"+class_unq).parents("form").append(jQuery(".modal.smsproModal[data-form-id="+form_id+"]"));
                                    }
                                });
                            }
    ...
    public function update_controls( $widget )
    {
        ...
        'sa_billing_phone' => [
                    ...
                    'label'        => esc_html__('Placeholder', 'soft-sms-alerts'),
        ...
        'label'        => esc_html__('Default Value', 'soft-sms-alerts'),
        'type'         => Elementor\Controls_Manager::TEXT,
        'default' => '',
        'dynamic' => [
         'active' => true,
...
class Sendmsms_Action_After_Submit extends \ElementorPro\Modules\Forms\Classes\Action_Base
{
    ...
    public function register_settings_section( $widget )
    {
        $widget->start_controls_section(
            'section_smspro',
            [
            'label' => __('Softeria Tech', 'soft-sms-alerts'),
            'condition' => [
        ...
        $widget->add_control(
            ...
            [
            'label' => __('OTP verification', 'soft-sms-alerts'),
        ...
        $widget->add_control(
            'customer_sms_enable',
            [
            'label' => __('Customer SMS', 'soft-sms-alerts'),
            'type' => \Elementor\Controls_Manager::SWITCHER,
            'label_on' => __('On', 'soft-sms-alerts'),
            'label_off' => __('Off', 'soft-sms-alerts'),
            'return_value' => 'true',
        ...
        $widget->add_control(
            'customer_message',
            [
            'label' => __('Customer Message', 'soft-sms-alerts'),
            'type' => \Elementor\Controls_Manager::TEXTAREA,
            'placeholder' => __('Write yout text or use fields shortcode', 'soft-sms-alerts'),
            'label_block' => true,
            'render_type' => 'none',
            'default' => SOFTSMAL_Messages::showMessage('DEFAULT_CONTACT_FORM_CUSTOMER_MESSAGE'),
            'classes' => '',
            'description' => __('Use fields shortcodes for send form data or write your custom text.', 'soft-sms-alerts'),
            ]
        );
        ...
        $widget->add_control(
            'admin_sms_enable',
            [
            'label' => __('Admin SMS', 'soft-sms-alerts'),
            'type' => \Elementor\Controls_Manager::SWITCHER,
            'label_on' => __('On', 'soft-sms-alerts'),
            'label_off' => __('Off', 'soft-sms-alerts'),
            'return_value' => 'true',
        ...
        $widget->add_control(
            'admin_number',
            [
            'label' => __('Admin Phone', 'soft-sms-alerts'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'placeholder' => __('8010551055', 'soft-sms-alerts'),
            'label_block' => true,
            'render_type' => 'none',
            'classes' => '',
            'description' => __('Send Message to admin on this number', 'soft-sms-alerts'),
            ]
        );
        ...
        $widget->add_control(
            'admin_message',
            [
            'label' => __('Admin Message', 'soft-sms-alerts'),
            'type' => \Elementor\Controls_Manager::TEXTAREA,
            'placeholder' => __('Write yout text or use fields shortcode', 'soft-sms-alerts'),
            'label_block' => true,
            'render_type' => 'none',
            'default' => SOFTSMAL_Messages::showMessage('DEFAULT_CONTACT_FORM_ADMIN_MESSAGE'),
            'classes' => '',
            'description' => __('Use fields shortcodes for send form data or write your custom text.', 'soft-sms-alerts'),
            'separator' => 'after',
