...
use Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController;
  ...
  * @link     https://sms.softeriatech.com/
  *
  * SOFTSMAL_WooCommerceCheckOutForm class
  */
class SOFTSMAL_WooCommerceCheckOutForm extends FormInterface
{
    /**
     ...
     */
    private $form_session_var = SOFTSMAL_FormSessionVars::WC_CHECKOUT;
     ...
     */
    private $form_session_var2 = SOFTSMAL_FormSessionVars::WC_CHECKOUT_POPUP;
     ...
     */
    private $form_session_var3 = SOFTSMAL_FormSessionVars::WC_POST_CHECKOUT;
	...
	public function modifyMessage($message, $type, $post_id) {
		...
		{
			$message = SOFTSMAL_WooCommerceCheckOutForm::pharseSmsBody(array('sms_body'=>$message), $post_id);
    ...
    public function refundedPartiallyAmount($order_id, $refundedid)
    {
        ...
        $buyerMessage         =  SOFTSMAL_WooCommerceCheckOutForm::pharseSmsBody($sms_data, $order_id);
        ...
        if (softeria_alerts_get_option('admin_notification_' . $new_status, 'softeria_alerts_general', 'on') === 'on' && ! empty($admin_phone_number) ) {
            ...
            $default_template           = SOFTSMAL_Messages::showMessage('DEFAULT_ADMIN_SMS_' . str_replace('-', '_', strtoupper($new_status)));
            $default_admin_sms          = ( ( ! empty($default_template) ) ? $default_template : SOFTSMAL_Messages::showMessage('DEFAULT_ADMIN_SMS_STATUS_CHANGED') );
            ...
            $adminMessage  =  SOFTSMAL_WooCommerceCheckOutForm::pharseSmsBody($admin_sms_data, $order_id);
    ...
    public function aeroCheckoutPage()
    {
		...
		jQuery(window).load(function(){
		  var modal = jQuery(".modal.smsproModal");
    ...
    public function woocommerceSiteCheckoutErrors( $data, $errors )
    {
        ...
        if (!$verify) {
            $errors->add('registration-error-invalid-nonce', __('Sorry, nonce did not verify.', 'soft-sms-alerts'));
        }
        if (! SOFTSMAL_Utility::isBlank(array_filter($errors->errors)) ) {
            return $errors;
        }
        ...
        if (SOFTSMAL_Utility::isBlank($user_phone)) {
            $errors->add('registration-error-invalid-phone', __('Please enter phone number.', 'soft-sms-alerts'));
        } else if (! isset($user_phone) || ! SOFTSMAL_Utility::validatePhoneNumber($user_phone) ) {
            global $phoneLogic;
        ...
        if ((isset($_POST['createaccount']) && $_POST['createaccount']) || $guest_checkout !== 'yes' && !is_user_logged_in()) {
            ...
            if (softeria_alerts_get_option('allow_multiple_user', 'softeria_alerts_general') !== 'on' ) {
                ...
                if (count($getusers) > 0 ) {
                    $errors->add('registration-error-number-exists', __('An account is already registered with this mobile number. Please login.', 'soft-sms-alerts'));
                }
            }
        }
    ...
    public function getCheckoutFields( $fields )
    {
        ...
        if (! empty($phone) ) {
            $_POST['billing_phone'] = SOFTSMAL_Utility::formatNumberForCountryCode($phone);
        }
        return $fields;
    }
    ...
    public function handleWoocommereCheckoutForm( $getdata )
    {
        ...
        if (! empty($_GET['option']) && sanitize_text_field(wp_unslash($_GET['option'])) === 'softeria-alert-woocommerce-post-checkout' ) {
            SOFTSMAL_Utility::initialize_transaction($this->form_session_var3);
        } else {
            SOFTSMAL_Utility::initialize_transaction($this->form_session_var);
        }
        $phone_num = SOFTSMAL_cURLOTP::checkPhoneNos($getdata['user_phone']);
    ...
    public function checkIfVerificationCodeNotEntered()
    {
        ...
        if (empty($_POST['order_verify']) ) {
            wc_add_notice(__('Your mobile number is not verified yet. Please verify your mobile number.', 'soft-sms-alerts'), 'error');
            return true;
        }
    }
    ...
    public function enableDisableScriptForButtonOnPage()
    {
		...
		jQuery(document).on("input change","input[name=billing_phone]",function() {
			...
			if (typeof phone != "undefined" && phone.replace(/\s+/g, "").match(' . esc_attr(SOFTSMAL_Constants::getPhonePattern()) . ') && (typeof jQuery(".sa_phone_error") == "undefined" || jQuery(".sa_phone_error").text()==""))  
			{
    ...
    public function handle_failed_verification( $user_login, $user_email, $phone_number )
    {
        ...
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            ...
            wc_add_notice(SOFTSMAL_Utility::_get_invalid_otp_method(), 'error');
        }
    }
    ...
    public function handleFormOptions()
    {
        ...
        if (is_plugin_active('woocommerce/woocommerce.php') ) {
            add_action('softsmal_addTabs', array( $this, 'addTabs' ), 1);
    ...
    public static function getCustomerTemplates()
    {
        ...
        $sms_body_new_note           = softeria_alerts_get_option('sms_body_new_note', 'softeria_alerts_message', SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_NOTE'));
        ...
        foreach ( $order_statuses as $ks  => $order_status ) {
            ...
            $default_template = SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_SMS_' . str_replace('-', '_', strtoupper($vs)));
            $text_body        = softeria_alerts_get_option('sms_body_' . $vs, 'softeria_alerts_message', ( ( ! empty($default_template) ) ? $default_template : SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_SMS_STATUS_CHANGED') ));
    ...
    public static function getMVAdminTemplates()
    {
        ...
        foreach ( $order_statuses as $ks  => $order_status ) {
            ...
            $default_template = SOFTSMAL_Messages::showMessage('DEFAULT_NEW_USER_' . str_replace('-', '_', strtoupper($vs)));
            $text_body        = softeria_alerts_get_option('multivendor_sms_body_' . $vs, 'softeria_alerts_message', ( ( ! empty($default_template) ) ? $default_template : SOFTSMAL_Messages::showMessage('DEFAULT_ADMIN_SMS_STATUS_CHANGED') ));
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $order_statuses as $ks  => $order_status ) {
            ...
            $default_template = SOFTSMAL_Messages::showMessage('DEFAULT_ADMIN_SMS_' . str_replace('-', '_', strtoupper($vs)));
            $text_body        = softeria_alerts_get_option('admin_sms_body_' . $vs, 'softeria_alerts_message', ( ( ! empty($default_template) ) ? $default_template : SOFTSMAL_Messages::showMessage('DEFAULT_ADMIN_SMS_STATUS_CHANGED') ));
    ...
    public static function pharseSmsBody( $sms_data, $order_id )
    {
        ...
        if (preg_match_all('/\[order_date.*?\]/', $content, $matched) ) {
            ...
            $date_params = SOFTSMAL_Utility::parseAttributesFromTag($date_tag);
    ...
    public function sendCustomSms( $data )
    {
        ...
        wp_send_json(SOFTSMAL_cURLOTP::sendsms($buyer_sms_data));
        exit();
    }
    ...
    public static function trigger_new_customer_note( $data )
    {
        ...
        if (softeria_alerts_get_option('buyer_notification_notes', 'softeria_alerts_general') === 'on' ) {
            ...
            $buyer_sms_body             = softeria_alerts_get_option('sms_body_new_note', 'softeria_alerts_message', SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_NOTE'));
            $buyer_sms_data             = array();
            $order       = wc_get_order($order_id );
			if ( version_compare( WC_VERSION, '7.1', '<' ) ) {
              $buyer_sms_data['number']   = get_post_meta( $order_id , '_billing_phone', true );
            ...
            $buyer_response = SOFTSMAL_cURLOTP::sendsms($buyer_sms_data);
            $response       = $buyer_response;

			if ('success' === $response['status'] ) {
                $order->add_order_note(__('Order note SMS Sent to buyer', 'soft-sms-alerts'));
            } else {
    ...
    public static function displaySendSmsMetaBox( $data )
    {
        ...
        wp_enqueue_script('admin-softeria-alert-scripts', SA_MOV_URL . 'js/admin.js', array( 'jquery' ), SOFTSMAL_Constants::SA_VERSION, true);
        ...
        ?>
                        ...
                        <option value=""><?php esc_html_e('Select Template', 'soft-sms-alerts'); ?></option>
        <?php
        ...
        ?>
                        ...
                        <p><a class="button tips" id="wc_softeria_alerts_sms_order_send_message" data-tip="<?php esc_html_e('Send an SMS to the billing phone number for this order.', 'soft-sms-alerts'); ?>"><?php esc_html_e('Send SMS', 'soft-sms-alerts'); ?></a>
    ...
    public static function trigger_after_order_place( $order_id, $old_status, $new_status )
    {
        ...
        if (in_array($new_status, $order_status_settings, true) && ( 0 === $order->get_parent_id() ) ) {
            $default_buyer_sms = defined('SOFTSMAL_Messages::DEFAULT_BUYER_SMS_' . str_replace(' ', '_', strtoupper($new_status))) ? constant('SOFTSMAL_Messages::DEFAULT_BUYER_SMS_' . str_replace(' ', '_', strtoupper($new_status))) : SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_SMS_STATUS_CHANGED');
            ...
            if ('success' === $response['status'] ) {
                $order->add_order_note(__('SMS Send to buyer Successfully.', 'soft-sms-alerts'));
            } else {
        ...
        if (softeria_alerts_get_option('admin_notification_' . $new_status, 'softeria_alerts_general', 'on') === 'on' && ! empty($admin_phone_number) ) {
            ...
            $admin_response = SOFTSMAL_cURLOTP::sendsms($admin_sms_data);
            $response       = $admin_response;
            if ('success' === $response['status'] ) {
                $order->add_order_note(__('SMS Sent Successfully.', 'soft-sms-alerts'));
            } else {
                if (is_array($response['description']) && array_key_exists('desc', $response['description']) ) {
                    $order->add_order_note($response['description']['desc']);
                } else {
    ...
    public function sendPostOrderOtp( $title = null, $order = array() )
    {
        ...
        if( !SOFTSMAL_cURLOTP::validateCountryCode($billing_phone)){
			return;
		}
        ...
        if ( !$post_verification ) {
            ...
            return __('Thank you, Your mobile number has been verified successfully.', 'soft-sms-alerts');
        }
    }
}
new SOFTSMAL_WooCommerceCheckOutForm();
?>
<?php
 /**
  * PHP version 5
        ...
        if (! empty($option) && ( 'fetch-order-variable' === sanitize_text_field($option) ) && ! empty($order_id) ) {
            ...
            $variables = array();
				...
				if(!empty($order_variables['meta_data']))
				{
					...
					unset($order_variables['meta_data']);
				}
				...
				if(!empty($order_variables['billing']))
				{
				 unset($order_variables['billing']);
				}
				if(!empty($order_variables['shipping']))
				{
				 unset($order_variables['shipping']);
				}
			}				
            ...
            $item_variables = SOFTSMAL_WooCommerceCheckOutForm::recursiveChangeKey($item_variables);
...
class SA_CodTOPrepaid
{
    ...
    { 
        ...
        $tabs['woocommerce']['inner_nav']['code_to_prepaid']['tab_section'] = 'smsprocarttemplates';
        $tabs['woocommerce']['inner_nav']['code_to_prepaid']['tabContent']  = $smsprocart_param;
        $tabs['woocommerce']['inner_nav']['code_to_prepaid']['filePath']    = 'views/cod-to-prepaid-setting-template.php';
        $tabs['woocommerce']['inner_nav']['code_to_prepaid']['params']      = $smsprocart_param;
        return $tabs;
    }
    ...
    public function getSmsAlertCodTemplates()
    {
        ...
        if (empty($scheduler_data) ) {
            ...
            'frequency' => '60',
            'message'   => SOFTSMAL_Messages::showMessage('DEFAULT_COD_PREPAID_CUSTOMER_MESSAGE'),  
            );
            $scheduler_data['cron'][] = array(
            'frequency' => '120',
            'message'   => SOFTSMAL_Messages::showMessage('DEFAULT_COD_PREPAID_CUSTOMER_MESSAGE'),
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $variables=SOFTSMAL_WooCommerceCheckOutForm::getvariables();
...
class All_Order_List extends WP_List_Table
{
    ...
    {
        $result = wc_get_orders(array('limit' => 5,'order' => 'DESC'));
		return $result;
    }
    ...
    {
        esc_html_e('No Order.', 'soft-sms-alerts');
    }
     ...
     * @return void
     */
    public function column_date_created_gmt( $item )
    {
         $date = date('d-m-Y', strtotime($item->get_date_created()));
        return $date; 
    }
...
{
    add_submenu_page('options.php', 'All Order Variable', 'All Order Variable', 'manage_options', 'all-order-variable', 'all_order_variable_page_handler');
}
