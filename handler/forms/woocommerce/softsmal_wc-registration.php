<?php
/**
 * This file handles SOFTSMAL_WooCommerceRegistrationForm via sms notification
 *
 * PHP version 5
 *
 * @category Handler
 ...
 * @link     https://sms.softeriatech.com/
 */
class SOFTSMAL_WooCommerceRegistrationForm extends FormInterface
{
    ...
    public function handleForm()
    {
        ...
        if ('on' === $buyer_signup_otp ) {
            ...
            if (is_plugin_active('dokan-lite/dokan.php') ) {
                add_action('dokan_reg_form_field', array( $this, 'smsproAddDokanPhoneField' ));
                add_action('dokan_vendor_reg_form_start', array( $this, 'smsproAddDokanPhoneField' ));
                add_action('dokan_vendor_reg_form_start', array( $this, 'smsproAddDokanVendorRegField' ));
            } else {
                add_action('woocommerce_register_form', array( $this, 'smsproAddPhoneField' ), 1);
            }

            if (is_plugin_active('dc-woocommerce-multi-vendor/dc_product_vendor.php') ) {
                add_action('wcmp_vendor_register_form', array( $this, 'smsproAddPhoneField' ));
            } 

            if ($this->popup_enabled ) {
                add_action('woocommerce_register_form_end', array( $this, 'smsproDisplayRegisterOTPBtn' ));
                if (is_plugin_active('easy-login-woocommerce/xoo-el-main.php') || is_plugin_active('easy-login-woocommerce-premium/xoo-el-main.php') ) {
                    add_action('xoo_el_register_add_fields', array( $this, 'smsproAddPhoneField' ));    
        ...
        if ('on' === $signup_with_mobile ) {
            if (is_plugin_active('easy-login-woocommerce/xoo-el-main.php') || is_plugin_active('easy-login-woocommerce-premium/xoo-el-main.php') ) {
				...
				if (!has_action('xoo_el_form_end', array($this, 'smsproDisplaySignupWithMobile'))) {
				  add_action('xoo_el_register_form_end', array( $this, 'smsproDisplaySignupWithMobile' ), 10);
				}
            }
            add_action('woocommerce_register_form_end', array( $this, 'smsproDisplaySignupWithMobile' ), 10);    
    ...
    public function getBillingFieldsProfilepage( $address, $load_address )
    {
        foreach ( $address as $key => $field ) {
            if ('billing_phone' === $key ) {
                $address['billing_phone']['value'] = SOFTSMAL_Utility::formatNumberForCountryCode($field['value']);
            }
        }
        return $address;
        // $args['value'] = SOFTSMAL_Utility::formatNumberForCountryCode( $value );
    }
    ...
    public function smsproDisplaySignupWithMobile($form = null, $args=array())
    {
        if ($form == null || $form == 'register') {
            echo wp_kses_post('<div class="lwo-container"><div class="sa_or">OR</div><button type="button" class="button sa_myaccount_btn" name="sa_myaccount_btn_signup" value="' . __('Signup with Mobile', 'soft-sms-alerts') . '" style="width: 100%; display:block"><span class="button__text">' . __('Signup with Mobile', 'soft-sms-alerts') . '</span></button></div>');
    ...
    public function validateWoocommerceSaveAddress( $user_id, $load_address, $address )
    {
        ...
        if ($db_billing_phone !== $user_phone ) {
            if (softeria_alerts_get_option('allow_multiple_user', 'softeria_alerts_general') !== 'on' && ! SOFTSMAL_Utility::isBlank($user_phone) ) {
                $_POST['billing_phone'] = SOFTSMAL_cURLOTP::checkPhoneNos($user_phone);

                $getusers = SOFTSMAL_Utility::getUsersByPhone('billing_phone', $user_phone, array( 'exclude' => array( $user_id ) ));
                if (count($getusers) > 0 ) {
    ...
    public function handleSignWthOtp()
    {
        ...
        if (!$verify) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response(__('Sorry, nonce did not verify.', 'soft-sms-alerts'), 'error'));
        }
        if (is_plugin_active('google-captcha/google-captcha.php')) {
            $check_result = apply_filters('gglcptch_verify_recaptcha', true, 'string', 'sa_swm_form');
            if (true !== $check_result ) { 
                wp_send_json(SOFTSMAL_Utility::_create_json_response(__('The reCaptcha verification failed. Please try again.', 'soft-sms-alerts'), 'error'));
            }
        }
        global $phoneLogic;
        ...
        if (isset($_REQUEST['option']) && sanitize_text_field(wp_unslash($_REQUEST['option']) === 'softeria-alert-registration-with-mobile') ) {
            ...
            $billing_phone = SOFTSMAL_cURLOTP::checkPhoneNos($phone_no);
            if (SOFTSMAL_Utility::isBlank($phone_no)) {
                wp_send_json(SOFTSMAL_Utility::_create_json_response(__('Please enter phone number.', 'soft-sms-alerts'), 'error'));
            } else if (! $billing_phone ) {

                $message = str_replace('##phone##', $phone_no, $phoneLogic->_get_otp_invalid_format_message());

                wp_send_json(SOFTSMAL_Utility::_create_json_response($message, 'error'));
            }
            ...
            if (is_wp_error($user) ) {
                $msg   = SOFTSMAL_Utility::_create_json_response(current($user->errors), 'error');
                wp_send_json($msg);
                exit();
            }  
            //-added for new user approve plugin
            SOFTSMAL_Utility::initialize_transaction($this->form_session_var3);
            softeria_alerts_site_challenge_otp(null, null, null, $billing_phone, 'phone', null, SOFTSMAL_Utility::currentPageUrl(), true);
        }
    }
    ...
    public function handleAjaxRegisterValidateOtp( $data )
    {
        ...
        if (strcmp($_SESSION['phone_number_mo'], $data['billing_phone']) ) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response(SOFTSMAL_Messages::showMessage('PHONE_MISMATCH'), 'error'));
        } else {
     ...
     * @return void
     */
    public static function smsproDisplayRegisterOTPBtn()
    {
    ...
    public function show_error_msg( $error_hook = null, $err_msg = null, $type = null )
    {
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            wp_send_json(SOFTSMAL_Utility::_create_json_response($err_msg, $type));
        } else {
     ...
     * @return void.
     */
    public function smsproAddPhoneField()
    {
        woocommerce_form_field(
            ...
            'label'       => SOFTSMAL_Messages::showMessage('Phone'),
            'id'       => 'reg_billing_phone',
            ),
            ( isset($_POST['billing_phone']) ? sanitize_text_field(wp_unslash($_POST['billing_phone'])) : '' )
        );
        remove_action('woocommerce_register_form', array( $this,'smsproAddPhoneField' ));
    }
     ...
     * @return void.
     */
    public function smsproAddDokanPhoneField()
    {
        $this->smsproAddPhoneField();
     ...
     * @return void.
     */
    public function smsproAddDokanVendorRegField()
    {
        ?>
        <script>
    ...
    public function processRegistration()
    {
        ...
        if (isset($_POST['softeria_alerts_name']) && $_POST['softeria_alerts_name']!='' && isset($_SESSION['sa_mobile_verified'])) {
            ...
            if (! is_wp_error($new_customer) ) {
                ...
                wp_send_json(
                    SOFTSMAL_Utility::_create_json_response(
                        'Please try again',
                        'success'
                    )
                );
...
new SOFTSMAL_WooCommerceRegistrationForm();