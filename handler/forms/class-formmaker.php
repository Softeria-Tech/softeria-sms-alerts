...
class SAFormMaker extends FormInterface
{
    ...
    public function addSmsalertShortcode($form)
    {
        ...
        $form_enable = softeria_alerts_get_option('formmarker_order_status_' . $form_id, 'softeria_alerts_formmarker_general', 'on'); 
        $otp_enable  = softeria_alerts_get_option('formmarker_otp_' . $form_id, 'softeria_alerts_formmarker_general', 'off');
        
        $phone_field = softeria_alerts_get_option('formmarker_sms_phone_' . $form_id, 'softeria_alerts_formmarker_general', '');
    ...
    public function formmakerSubmissionComplete($form )
    {
        ...
        $phone_field      = softeria_alerts_get_option('formmarker_sms_phone_' . $form_id, 'softeria_alerts_formmarker_general', ''); 
        $buyer_sms_notify = softeria_alerts_get_option('formmarker_message_' . $form_id, 'softeria_alerts_formmarker_general', 'on'); 
        $admin_sms_notify = softeria_alerts_get_option('formmarker_admin_notification_' . $form_id, 'softeria_alerts_formmarker_general', 'on'); 
        if ('on' === $form_enable && 'on' === $buyer_sms_notify) {
            $buyer_sms_content = softeria_alerts_get_option('formmarker_sms_body_' . $form_id, 'softeria_alerts_formmarker_message', '');          
            $datas = $form['fvals'] ;
        ...
        if ('on' === $form_enable && 'on' === $admin_sms_notify ) {
            ...
            if (! empty($admin_phone_number) ) {
                $admin_sms_content = softeria_alerts_get_option('formmarker_admin_sms_body_'. $form_id, 'softeria_alerts_formmarker_message', '');
    ...
    public static function addTabs( $tabs = array() )
    {
        ...
        'kb_link'      => array(
        'href'   => 'https://sms.softeriatech.com/knowledgebase/form-maker-sms-integration/',
        'target' => '_blank',
    ...
    public static function add_default_settings( $defaults = array() )
    {
        ...
        foreach ( $wpam_statuses as $ks => $vs ) {
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_admin_notification_' . $ks ] = 'off';
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_order_status_' . $ks ]       = 'off';
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_message_' . $ks ]            = 'off';
            $defaults['softeria_alerts_formmarker_message'][ 'formmarker_admin_sms_body_' . $ks ]     = '';
            $defaults['softeria_alerts_formmarker_message'][ 'formmarker_sms_body_' . $ks ]           = '';
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_sms_phone_' . $ks ]          = '';
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_sms_otp_' . $ks ]            = '';
            $defaults['softeria_alerts_formmarker_general'][ 'formmarker_otp_' . $ks ]                = '';
            $defaults['softeria_alerts_formmarker_message'][ 'formmarker_otp_sms_' . $ks ]            = '';
        }
        return $defaults;
    }
