...
class UltimateMemberRegistrationForm extends FormInterface
{
    ...
    public function handleSmsalertChangedPwd( $post_data )
    {
        ...
        if (! empty($error) ) {
            smsproAskForResetPassword($_SESSION['user_login'], $_SESSION['phone_number_mo'], $error, 'phone', false);
        }
     ...
     * @return object
     */
    public function smsproUmResetPwdSubmitted( $datas )
    {
     ...
     * @return void
     */
    public function smsproUmRegistrationValidation( $args )
    {
        SmsAlertUtility::checkSession();
