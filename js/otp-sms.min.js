...
$sa(document).on(
    "input", ".otp_input",function () {
		var otp_length     = $sa('#smspro_customer_validation_otp_token').attr('data-max');
...
$sa(document).on(
    "click",".sa_default_login_form,.sa_default_signup_form",function () {
        ...
        if($sa(this).parents('.smspro-modal').length>0) {
            var id = $sa(this).parents('.smspro-modal').attr('id');
...
function hideOtpModal(obj)
{
	...
	if ("on" === otp_in_popup ) {
	 obj.parents(".smsproModal").not('.smspro-modal').hide('slow');
	}
	else{
	...
	if(modal_style != '')
	{
		setTimeout(
			function () {
				...
				if ("on" !== otp_in_popup ) {	
					obj.parents(".smsproModal").not('.smspro-modal').hide();
				}
			}, 1000
		); 
	}
...
{
	$sa('.smsproModal .smspro_validate_field,.smsproModal .saeditphone,.smsproModal .sa-message').show();
...
function otpSuceess(o,current_btn,callback)
{
    ...
    (
	current_modal.find(".smspro_validate_field").hide(),
...
function add_smspro_button(submit_selector,phone_selector,unique_id,button_text='')
{
    if(!$sa(submit_selector).hasClass("sa-default-btn-hide") && ($sa(phone_selector).attr('type') !== 'hidden' || $sa(phone_selector).parents("form").hasClass('woocommerce-post-checkout-form'))) {
        ...
        $sa(button.first().clone()).insertAfter(button.first()).addClass("sa-otp-btn-init smspro_otp_btn_submit");
...
function send_otp(obj,submit_selector,phone_selector,username_selector,password_selector)
{
    ...
    if($sa(obj).parents("form").find(".sa_field_error").length === 0 && verification_allowed) {
        if(username_selector !="" && password_selector !="") {
            var action_url     = site_url+"/?option=smspro_ajax_login_popup";
            ...
            action_url     = site_url+"/?option=smspro_ajax_login_with_otp";
        }
        else if($sa(obj).parents("form").hasClass("sa-signupwithotp-form")) {
            action_url     = site_url+"/?option=smspro-registration-with-mobile";
        }
            ...
            action_url     = site_url+"/?option=smspro-woocommerce-checkout-process&wc-ajax=checkout"+cartflow_param;
                }
        else if($sa(obj).parents("form").hasClass("woocommerce-post-checkout-form")) {
			data     = {user_phone:e};
            action_url     = site_url+"/?option=smspro-woocommerce-post-checkout";
        }
			...
			action_url     = site_url+"/?option=smspro_register_with_otp";
			if($sa(obj).parents(".nasa_register-form").length > 0)
			{
				...
				action_url     = sa_otp_settings['ajax_url']+"?option=smspro_register_with_otp";	
			}
            
        }
        else if($sa(obj).parents("form").hasClass("elementor-form")) {
            data = data+"&action=elementor_pro_forms_send_form";
            action_url     = sa_otp_settings['ajax_url']+"?option=smspro_elementor_form_otp";
        }
		else if($sa(obj).parents("form").hasClass("wpcf7-form")) {
            action_url     = site_url+"/?option=smspro_wpcf7_form_otp";
        }
		else if($sa(obj).parents("form#wpas_form_registration").length > 0) {
            action_url     = site_url+"/?option=smspro_wpas_form_otp";
        }
            ...
            action_url     = site_url+"/?option=smspro_frm_show_form_otp";
        }
        else if($sa(obj).parents("form").hasClass("wpforms-form")) 
		{
			var submitsAWpForm = function() {
				...
				action_url     = site_url+"/?option=smspro_wpforms_otp";
				saInitOTPProcess(
			...
			data     = {user_phone:e};
            action_url     = site_url+"/?option=smspro-shortcode-ajax-verify";
        }
...
function validateOtp(event)
{
    ...
    var otp = $sa("#smspro_customer_validation_otp_token").val();
    var action_url         = site_url+"/?option=smspro-validate-otp-form";
	var data = '';
	...
	{
		data  = "otp_type=phone&from_both=&smspro_customer_validation_otp_token="+otp;
	}
	else{
       data  = c_form.serialize()+"&otp_type=phone&from_both=&smspro_customer_validation_otp_token="+otp;
    }
    sa_validateOTP(
...
if ('OTPCredential' in window) {
    window.addEventListener(
        'DOMContentLoaded', e => {
            const input = document.querySelector('#smspro_customer_validation_otp_token');
    ...
    try {
        navigator.credentials.get(
            ...
            if($sa(".otp-number").hasClass('hide')) {
                $sa("#smspro_customer_validation_otp_token").val(otp.code);  
            }          
            else{
                pasteOtp(otp.code);  
            }
...
function saEditNumber(event){ 
    ...
    $sa('.saeditphone,.smsproModal .smspro_validate_field').hide();
    var otp_in_popup = sa_otp_settings['otp_in_popup'];
	var currentModel = $sa(".modal.smsproModal");
	if ("on" === otp_in_popup ) {                          
		currentModel.find(".ring").removeClass('sa-hide');    
    ...
    { 
		...
		$sa(".sa-edit-phone form").append("<button type='button' class='button saresubmit smspro_otp_btn_submit' onclick='saReSubmit(this)'>Request Otp</button>");
	}
	...
	$sa('.sa-edit-phone,.sa-edit-phone .smspro_otp_btn_submit').show();
