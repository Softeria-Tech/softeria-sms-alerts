...
$sa(document).on(
    "input", ".otp_input",function () {
		var otp_length     = $sa('#softeria_alerts_customer_validation_otp_token').attr('data-max');
...
$sa(document).on(
    "click",".sa_default_login_form,.sa_default_signup_form",function () {
        ...
        if($sa(this).parents('.softeria-alert-modal').length>0) {
            var id = $sa(this).parents('.softeria-alert-modal').attr('id');
...
function hideOtpModal(obj)
{
    ...
    var modal_style = obj.parents('.smsproModal').attr('data-modal-close');
    obj.parents('.smsproModal').addClass(modal_style+'Out').removeClass(modal_style+'In');
	var otp_in_popup = sa_otp_settings['otp_in_popup'];
	if ("on" === otp_in_popup ) {
	 obj.parents(".smsproModal").not('.softeria-alert-modal').hide('slow');
	}
	else{
		 obj.parents('.smsproModal').next().addClass(modal_style+"Up").removeClass(modal_style+'Down');
		if(modal_style != '' && modal_style != 'modal-slide')
		{ 
		 obj.parents('.smsproModal').parent().css('min-height',obj.parents('.smsproModal').next().height());
		}
		obj.parents('.smsproModal').next().show();
	}
	if(modal_style != '')
	{
		setTimeout(
			function () {
				$sa('.smsproModal').removeClass(modal_style+'Out').addClass(modal_style+'In');
				if ("on" !== otp_in_popup ) {	
					obj.parents(".smsproModal").not('.softeria-alert-modal').hide();
				}
			}, 1000
		); 
	}
...
$sa(document).on(
    "keyup",".smsproModal .otp-number",function (e) {
        var otp_length     = $sa('#softeria_alerts_customer_validation_otp_token').attr('data-max');
        var txtData     = [];
        var parent         = $sa(this).parents(".smsproModal");
...
function saInitOTPProcess(obj,action_url, data_obj,otp_resend_timer=15,success_cb=null,failure_cb=null,phone_selector=null,submit_selector=null)
{
	...
	var otp_in_popup = sa_otp_settings['otp_in_popup'];
    if ("on" === otp_in_popup ) {
		var currentModel     = $sa(".modal.smsproModal");
...
{
	$sa('.smsproModal .softeria_alerts_validate_field,.smsproModal .saeditphone,.smsproModal .sa-message').show();
	$sa('.saresubmit ').removeClass('button--loading').attr("disabled",false);
    $sa('.smsproModal .sa-edit-phone').hide();
	...
	{                 
        $sa('.smsproModal .edit-user-phone').hide();        
    }
    else{    
        $sa('.smsproModal .edit-user-phone').show();        
    }
    ("success"==o.result)?(
...
function sa_validateOTP(obj,action_url,data_obj,callback)
{
    ...
    var current_modal = $sa(".modal.smsproModal");
	var otp_in_popup = sa_otp_settings['otp_in_popup'];
    if ("on" === otp_in_popup ) {
...
function otpSuceess(o,current_btn,callback)
{
    ...
    (
	current_modal.find(".softeria_alerts_validate_field").hide(),
...
function add_softeria_alerts_button(submit_selector,phone_selector,unique_id,button_text='')
{
    if(!$sa(submit_selector).hasClass("sa-default-btn-hide") && ($sa(phone_selector).attr('type') !== 'hidden' || $sa(phone_selector).parents("form").hasClass('woocommerce-post-checkout-form'))) {
        ...
        $sa(button.first().clone()).insertAfter(button.first()).addClass("sa-otp-btn-init softeria_alerts_otp_btn_submit");
...
{
    $sa(obj).parents(".smsproModal").hide();
    var country_enable = sa_otp_settings['show_countrycode'];
    ...
    if($sa(obj).parents("form").find(".sa_field_error").length === 0 && verification_allowed) {
        if(username_selector !="" && password_selector !="") {
            var action_url     = site_url+"/?option=softeria_alerts_ajax_login_popup";
            ...
            action_url     = site_url+"/?option=softeria_alerts_ajax_login_with_otp";
        }
        else if($sa(obj).parents("form").hasClass("sa-signupwithotp-form")) {
            action_url     = site_url+"/?option=softeria-alert-registration-with-mobile";
        }
            ...
            action_url     = site_url+"/?option=softeria-alert-woocommerce-checkout-process&wc-ajax=checkout"+cartflow_param;
                }
        else if($sa(obj).parents("form").hasClass("woocommerce-post-checkout-form")) {
			data     = {user_phone:e};
            action_url     = site_url+"/?option=softeria-alert-woocommerce-post-checkout";
        }
			...
			action_url     = site_url+"/?option=softeria_alerts_register_with_otp";
			if($sa(obj).parents(".nasa_register-form").length > 0)
			{
				...
				action_url     = sa_otp_settings['ajax_url']+"?option=softeria_alerts_register_with_otp";	
			}
            
        }
        else if($sa(obj).parents("form").hasClass("elementor-form")) {
            data = data+"&action=elementor_pro_forms_send_form";
            action_url     = sa_otp_settings['ajax_url']+"?option=softeria_alerts_elementor_form_otp";
        }
		else if($sa(obj).parents("form").hasClass("wpcf7-form")) {
            action_url     = site_url+"/?option=softeria_alerts_wpcf7_form_otp";
        }
		else if($sa(obj).parents("form#wpas_form_registration").length > 0) {
            action_url     = site_url+"/?option=softeria_alerts_wpas_form_otp";
        }
            ...
            action_url     = site_url+"/?option=softeria_alerts_frm_show_form_otp";
        }
        else if($sa(obj).parents("form").hasClass("wpforms-form")) 
		{
			var submitsAWpForm = function() {
				...
				action_url     = site_url+"/?option=softeria_alerts_wpforms_otp";
				saInitOTPProcess(
			...
			data     = {user_phone:e};
            action_url     = site_url+"/?option=softeria-alert-shortcode-ajax-verify";
        }
...
function validateOtp(event)
{
    ...
    var otp = $sa("#softeria_alerts_customer_validation_otp_token").val();
    var action_url         = site_url+"/?option=softeria-alert-validate-otp-form";
	var data = '';
	...
	{
		data  = "otp_type=phone&from_both=&softeria_alerts_customer_validation_otp_token="+otp;
	}
	else{
       data  = c_form.serialize()+"&otp_type=phone&from_both=&softeria_alerts_customer_validation_otp_token="+otp;
    }
    sa_validateOTP(
...
if ('OTPCredential' in window) {
    window.addEventListener(
        'DOMContentLoaded', e => {
            const input = document.querySelector('#softeria_alerts_customer_validation_otp_token');
    ...
    try {
        navigator.credentials.get(
            ...
            if($sa(".otp-number").hasClass('hide')) {
                $sa("#softeria_alerts_customer_validation_otp_token").val(otp.code);  
            }          
            else{
                pasteOtp(otp.code);  
            }
...
function saEditNumber(event){ 
    ...
    $sa('.smsproModal .sa-message').attr("style", "display: none !important");
    $sa('.saeditphone,.smsproModal .softeria_alerts_validate_field').hide();
    var otp_in_popup = sa_otp_settings['otp_in_popup'];
	var currentModel = $sa(".modal.smsproModal");
	if ("on" === otp_in_popup ) {                          
		currentModel.find(".ring").removeClass('sa-hide');    
    ...
    { 
		...
		$sa(".sa-edit-phone form").append("<button type='button' class='button saresubmit softeria_alerts_otp_btn_submit' onclick='saReSubmit(this)'>Request Otp</button>");
	}
	...
	{
        var modal_style = currentModel.attr("data-modal-close");
        $sa(".modal.smsproModal").addClass(modal_style+"In");
	...
	$sa('.sa-edit-phone,.sa-edit-phone .softeria_alerts_otp_btn_submit').show();
