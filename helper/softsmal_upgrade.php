 ...
 * @license  URI: http://www.gnu.org/licenses/gpl-2.0.html
 * @link     https://sms.softeriatech.com/
 * SOFTSMAL_SAUpgrade class
 */
class SOFTSMAL_SAUpgrade
{
    ...
    public static function smsproUpgrade()
    {
        ...
        $plugin_version = SOFTSMAL_Constants::SA_VERSION;
        ...
        if ($db_version <= '3.5.3' && !SOFTSMAL_Utility::isPlayground()) {
            global $wpdb;
		...
		if ($db_version <= '3.7.5' && !SOFTSMAL_Utility::isPlayground() && $senderid == 'ESTORE') {
            $sms_otp_send  = softeria_alerts_get_option('sms_otp_send', 'softeria_alerts_message');
			if($sms_otp_send != '')
			{
				$output   = get_option('softeria_alerts_message');
                $output['sms_otp_send'] = SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_OTP');
                update_option('softeria_alerts_message', $output);
			}
        }
