 ...
 * @license  URI: http://www.gnu.org/licenses/gpl-2.0.html
 * @link     https://sms.softeriatech.com/
 * SOFTSMAL_Utility class
 */
class SOFTSMAL_Utility
{
    ...
    public static function validatePhoneNumber( $phone )
    {
        ...
        if (! preg_match(SOFTSMAL_Constants::getPhonePattern(), $phone, $matches) ) {
            return false;
        } else {
            return true;
        }
    ...
    {
        self::checkSession();
        $reflect = new ReflectionClass('SOFTSMAL_FormSessionVars');
    ...
    {
        return SOFTSMAL_CURLOTP::showMessage('INVALID_OTP');
    }
    ...
    public static function enqueue_script_for_intellinput()
    {
        if ('on' === softeria_alerts_get_option('checkout_show_country_code', 'softeria_alerts_general') ) {
            ...
            wp_enqueue_script('sa_pv_intl-phones-lib', SA_MOV_URL . 'js/intlTelInput-jquery.min.js', $dep, SOFTSMAL_Constants::SA_VERSION, true);
            wp_enqueue_script('wccheckout_utils', SA_MOV_URL . 'js/utils.js', array( 'jquery' ), SOFTSMAL_Constants::SA_VERSION, true);
            wp_enqueue_script('wccheckout_default', SA_MOV_URL . 'js/phone-number-validate.js', array( 'sa_pv_intl-phones-lib' ), SOFTSMAL_Constants::SA_VERSION, true);
            ...
            wp_enqueue_style('wpv_telinputcss_style', SA_MOV_URL . 'css/intlTelInput.min.css', array(), SOFTSMAL_Constants::SA_VERSION, false);
        }
    }
    ...
    public static function getUsersByPhone( $key, $value, $extra_datas = array() )
    {
        if (empty($value) ) {
            ...
            $wcc_ph     = SOFTSMAL_cURLOTP::checkPhoneNos($value);
            $wocc_ph    = SOFTSMAL_cURLOTP::checkPhoneNos($value, false);
            $wth_pls_ph = '+' . $wcc_ph;
    ...
    public static function formatNumberForCountryCode( $phoneNum )
    {
        ...
        if ('on' === $country_code_enabled && ! empty($phoneNum) ) {
            $phone = SOFTSMAL_cURLOTP::checkPhoneNos($phoneNum);
            return ($phone!='')?'+' . SOFTSMAL_cURLOTP::checkPhoneNos($phoneNum):$phoneNum;
        } else {
            return $phoneNum;
        }
    }
