...
class SAVerify
{
    ...
    private $formSessionVar       = SOFTSMAL_FormSessionVars::SA_SHORTCODE_FORM_VERIFY;
    ...
    public function _send_otp_shortcode_ajax_verify( $getdata )
    {
        global $phoneLogic;
        SOFTSMAL_Utility::checkSession();
        SOFTSMAL_Utility::initialize_transaction($this->formSessionVar);

        $phone = SOFTSMAL_CURLOTP::checkPhoneNos($getdata['user_phone']);
        if (array_key_exists('user_phone', $getdata) && ! SOFTSMAL_Utility::isBlank($getdata['user_phone']) && ! empty($phone) ) {
            $_SESSION[ $this->formSessionVar ] = $phone;
            softeria_alerts_site_challenge_otp('test', null, null, $phone, 'phone', null, null, 'ajax');
        } else {
            if (SOFTSMAL_Utility::isBlank($getdata['user_phone'])) {
    ...
    public static function enqueue_otp_js_script()
    {
        ...
        wp_enqueue_script("sa-handle-footer", SA_MOV_URL . 'js/otp-sms.min.js', array( 'jquery' ), SOFTSMAL_Constants::SA_VERSION, true); //SOFTSMAL_Constants::SA_VERSION, true );
        
        wp_enqueue_style('sa-login-css', SA_MOV_CSS_URL, array(), SOFTSMAL_Constants::SA_VERSION, false);
        ...
        $auto_validate = !empty(SOFTSMAL_Utility::get_elementor_data("auto_validate"))?SOFTSMAL_Utility::get_elementor_data("auto_validate"):SOFTSMAL_Utility::get_elementor_data('auto_validate', 'softeria_alerts_general', 'off');
        wp_localize_script(
            'sa-handle-footer',
            'sa_otp_settings',
            array(
            ...
            'valid_otp'                => SOFTSMAL_Messages::showMessage('VALID_OTP'),
