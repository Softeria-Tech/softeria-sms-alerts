 ...
 * @license  URI: http://www.gnu.org/licenses/gpl-2.0.html
 * @link     https://sms.softeriatech.com/
 * SOFTSMAL_CURLOTP class 
 */
class SOFTSMAL_CURLOTP
{
        ...
        if (is_array($nos) ) {
            foreach ( $nos as $no ) {
                ...
                if (! empty($no) ) {
                    ...
                    $match = preg_match(SOFTSMAL_Constants::getPhonePattern(), $no);
                    if ($match ) {
                        $valid_no[] = $no;
                    }
                    //}
    ...
    public static function smsproSendOtpToken( $form, $email = '', $phone = '' )
    {
        ...
        $password = softeria_alerts_get_option('softeria_alerts_password', 'softeria_alerts_gateway');
        $senderid = softeria_alerts_get_option('softeria_alerts_api', 'softeria_alerts_gateway');
        $template = softeria_alerts_get_option('sms_otp_send', 'softeria_alerts_message', SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_OTP'));
    ...
    public static function callAPI( $url, $params, $headers = array( 'Content-Type: application/json' ) )
    {
        ...
        'version' =>SOFTSMAL_Constants::SA_VERSION
        );
