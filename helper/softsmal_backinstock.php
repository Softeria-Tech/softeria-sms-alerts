...
class SOFTSMAL_Backinstock
{
     ...
     * @return void
     */
    public function smsproPopulateSubscriber( $column_name, $product_id )
    {
    ...
    public function enqueueScriptOnPage()
    {
        ...
        wp_register_script('sa_single_product', SA_MOV_URL . 'js/wc-product.js', array( 'jquery' ), SOFTSMAL_Constants::SA_VERSION, true);
    ...
    public static function getBackInStockTemplates()
    {
        ...
        $text_body        = softeria_alerts_get_option(
            ...
            SOFTSMAL_Messages::showMessage('DEFAULT_BACK_IN_STOCK_CUST_MSG')
        );
        ...
        $text_body        = softeria_alerts_get_option(
            ...
            'softeria_alerts_bis_message',
            SOFTSMAL_Messages::showMessage('DEFAULT_BACK_IN_STOCK_SUBSCRIBE_MSG')
        );
    ...
    {
        if (! empty($data) ) {
            if (isset($data['action']) && 'smsprobackinstock' === $data['action'] ) {
    ...
    public function performActionOnAjaxData( $post_data )
    {
        ...
        $subscriber_phone = SOFTSMAL_cURLOTP::checkPhoneNos($user_phone);
        if (! $subscriber_phone ) {
            $data['status']      = 'error';
            ...
            if (! empty($check_is_already_subscribed) ) {
                ...
                if ('' !== $subscriber_phone ) {
                    ...
                    if ('on' === $subscribed_bis_notify && '' !== $subscribed_message ) {
                        ...
                        SOFTSMAL_cURLOTP::sendsms($buyer_sms_data);
                    }
                    $data['status']      = 'success';
    ...
    public function isAlreadySubscribed( $product_id, $variation_id, $subscriber_phone, $get_user_id )
    {
        ...
        $wcc_ph     = SOFTSMAL_cURLOTP::checkPhoneNos($subscriber_phone);
        $wocc_ph    = SOFTSMAL_cURLOTP::checkPhoneNos($subscriber_phone, false);
        $wth_pls_ph = '+' . $wcc_ph;
...
{
    add_submenu_page('options.php', 'All Subscriber', 'All Subscriber', 'manage_options', 'all-subscriber', 'subscriberPageHandler');
}
