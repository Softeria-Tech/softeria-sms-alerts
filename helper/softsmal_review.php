...
class WCReview
{
    ...
    public function scheduleSms( $order_id, $old_status, $new_status, $instance )
    {
        ...
        if ($new_status === $message_status && 'on' === $customer_notify && '' !== $review_message && 0 === $order->get_parent_id() ) {
            ...
            if ('on' === $time_enabled ) {
                ...
                $date_modified = SOFTSMAL_Utility::sa_date_time($order->get_date_modified(), 'Y-m-d');
                $default_time  = $date_modified . ' ' . $schedule_time;
                $schedule      = SOFTSMAL_Utility::sa_date_time($default_time, 'Y-m-d H:i:s', $days . ' days');
                $ist           = SOFTSMAL_Utility::date_time_ist($schedule);
            } else {
                $order_time = SOFTSMAL_Utility::date_time_ist();
                $schedule   = SOFTSMAL_Utility::sa_date_time($order_time, 'Y-m-d H:i:s', $days . ' days');
            }
    ...
    public static function getReviewTemplates()
    {
        ...
        $text_body                      = softeria_alerts_get_option('customer_notify', 'softeria_alerts_or_message', SOFTSMAL_Messages::showMessage('DEFAULT_CUSTOMER_REVIEW_MESSAGE'));
        $review_added_user_msg          = softeria_alerts_get_option('review_added_user_msg', 'softeria_alerts_review', 'on');
        ...
        'moreoption'     => 1,
        'token'          => SOFTSMAL_WooCommerceCheckOutForm::getvariables(),
        );
        $templates = array();
