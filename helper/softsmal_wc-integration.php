...
if (is_plugin_active('woocommerce-shipment-tracking/woocommerce-shipment-tracking.php')
    ...
    new SOFTSMAL_ShipmentIntegration();
}
/**
 * PHP version 5
 *
 ...
 * @license  URI: http://www.gnu.org/licenses/gpl-2.0.html
 * @link     https://sms.softeriatech.com/
 * SOFTSMAL_ShipmentIntegration class
 */
class SOFTSMAL_ShipmentIntegration
{
    ...
    public function replaceWcAdvshipmentTrackingno( $sms_data, $order_id )
    {
        if (is_plugin_active('woo-advanced-shipment-tracking/woocommerce-advanced-shipment-tracking.php')
            ...
            if (preg_match_all('/\[date_shipped.*?\]/', $content, $matched) ) {
                ...
                $date_params     = SOFTSMAL_Utility::parseAttributesFromTag($date_shipped);
    ...
    public function triggerOrderTrackship( $order_id )
    {
        ...
        if (in_array($order_status, $order_status_settings, true)  ) {
            $default_buyer_sms = defined('SOFTSMAL_Messages::DEFAULT_BUYER_SMS_' . str_replace(' ', '_', strtoupper($order_status))) ? constant('SOFTSMAL_Messages::DEFAULT_BUYER_SMS_' . str_replace(' ', '_', strtoupper($order_status))) : SOFTSMAL_Messages::showMessage('DEFAULT_BUYER_SMS_STATUS_CHANGED');
            ...
            $buyer_sms_data             = SOFTSMAL_WooCommerceCheckOutForm::pharseSmsBody($buyer_sms_data, $order_id);
    ...
    public function replaceTokenWcTemplates( $sms_data, $order_id )
    {
        if (is_plugin_active('wc-serial-numbers/wc-serial-numbers.php') ) {
			...
			if(class_exists('WC_Serial_Numbers_Query'))
			{
              ...
              $serial_numbers = wcsn_order_get_keys($order_id);
			}
...
class SAWCAuctions
{
    ...
    public function __construct()
    {
        ...
        add_action('softsmal_addTabs', array( $this, 'addTabs' ), 100);
    }
...
class SAWCDokan
{
    ...
    public function sendApprovedSmsVendor( $user_id )
    {
        ...
        if ('on' === $enabled && ! empty($author_no) ) {
            $content = softeria_alerts_get_option('multivendor_sms_body_approved', 'softeria_alerts_message', SOFTSMAL_Messages::showMessage('DEFAULT_NEW_USER_APPROVED'));
    ...
    public function sendRejectedSmsVendor( $user_id )
    {
        ...
        if ('on' === $enabled && ! empty($author_no) ) {
            $content = softeria_alerts_get_option('multivendor_sms_body_rejected', 'softeria_alerts_message', SOFTSMAL_Messages::showMessage('DEFAULT_NEW_USER_REJECTED'));
            do_action('sa_send_sms', $author_no, $this->parseSmsBody($content, $user_id));
        }
    }
...
class SARequestQuote
{
    ...
    {
        SOFTSMAL_WooCommerceCheckOutForm::trigger_after_order_place($order_id, 'ywraq-new', 'ywraq-new');
    }
}
...
class SAReminderlist
{
    ...
    {
		if (! class_exists('SA_Admin_Reminder_Table') ) {
			return;
		}
            global $wpdb, $pagenow;
            $table_name = $wpdb->prefix . 'softeria_alerts_booking_reminder';
