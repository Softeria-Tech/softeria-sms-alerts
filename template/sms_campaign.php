    ...
    foreach ( $post_ids as $key => $post_id ) {
        if ($type == 'orders_data') {
            ...
            if (version_compare(WC_VERSION, '7.1', '<') ) {
                ...
                $user_phone  = !empty($order->get_billing_phone())?$order->get_billing_phone():$order->get_shipping_phone(); 
            }
            $phone[] =$user_phone;
        ...
        <div id="send_section" style="<?php echo $send_hide; ?>">
            ...
            <div  id="sendbox_section">
                <p class="sarow">
                    ...
                    <select id="senderid">
                   <?php
                    if (!empty($senderids['status'])) {
                        foreach ($senderids['data'] as $senderid) {?>
                            <option value="<?php  echo $senderid;?>"><?php  echo $senderid;?></option>                           
                        <?php }
                    } else {?>
                <option value="ESTORE">ESTORE</option>  
                    ...
                    <!-- <p class="sarow">
                        <label> Credits:<br>
                        <select id="route">                       
                        <?php //foreach ($credits as $key => $route) { ?>
