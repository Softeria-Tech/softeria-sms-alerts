								...
								<div class="sa_customer_validation-modal-dialog sa_customer_validation-modal-md">
									<div class="login sa_customer_validation-modal-content">
										<div class="sa_customer_validation-modal-header">
											<b>' . esc_html__('Validate your Phone Number', 'sms-pro') . '</b>
											<a class="go_back" href="#" onclick="window.location =\'' . esc_attr($go_back_url) . '\'" > &larr;
												' . esc_html__('Go Back', 'sms-pro') . '</a>
										</div>
										<div class="sa_customer_validation-modal-body center">
											<div id="message">' . esc_html__($message, 'sms-pro') . '</div><br /> ';
if (! SmsAlertUtility::isBlank($user_email) ) {
    echo '									<div class="sa_customer_validation-login-container">
													<form name="f" id="validate_otp_form" method="post" action="">
														<input id="validate_phone" type="hidden" name="option" value="smspro_ajax_form_validate" />
														...
														<input type="text" name="sa_phone_number"  autofocus="true" placeholder="" 
															...
															title="' . esc_html__('Enter a number in the following format', 'sms-pro') . ': 9xxxxxxxxx"/>
														...
														<div id="sa_validate_otp" hidden>
															' . esc_html__('Verify Code ', 'sms-pro') . ' <input type="text" 
															name="smspro_customer_validation_otp_token"  autofocus="true" placeholder="" 
															id="smspro_customer_validation_otp_token" required="true" 
														...
														<input type="button" hidden id="validate_otp" name="otp_token_submit" 
															class="smspro_otp_token_submit"  value="Validate" />
														<input type="button" id="send_otp" class="smspro_otp_token_submit" 
							...
							jQuery(document).ready(function() {
							    jQuery("#send_otp").click(function(o) {
							        ...
							        jQuery("#salert_message").empty(), jQuery("#salert_message").append("' . wp_kses_post($img) . '"), jQuery("#salert_message").show(), jQuery.ajax({
							            url: "' . esc_attr(site_url()) . '/?option=smspro-ajax-otp-generate",
							            type: "POST",
								...
								jQuery("#validate_otp").click(function(o) {
							        var e = jQuery("input[name=smspro_customer_validation_otp_token]").val();
									if( typeof sa_otp_settings !=  "undefined" && sa_otp_settings["show_countrycode"] == "on" ){
							        ...
							        jQuery("#salert_message").empty(), jQuery("#salert_message").append("' . wp_kses_post($img) . '"), jQuery("#salert_message").show(), jQuery.ajax({
							            url: "' . esc_attr(site_url()) . '/?option=smspro-ajax-otp-validate",
							            type: "POST",
							            data: {smspro_customer_validation_otp_token: e,billing_phone:f,redirect_to:r},
							            crossDomain: !0,
							            dataType: "json",
							            success: function(o) {
							                if (o.result == "success") {
