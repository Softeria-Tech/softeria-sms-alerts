							...
							jQuery(document).ready(function() {
							    jQuery("#send_otp").click(function(o) {
							        ...
							        jQuery("#salert_message").empty(), jQuery("#salert_message").append("' . wp_kses_post($img) . '"), jQuery("#salert_message").show(), jQuery.ajax({
							            url: "' . esc_attr(site_url()) . '/?option=softeria-alert-ajax-otp-generate",
							            type: "POST",
								...
								jQuery("#validate_otp").click(function(o) {
							        var e = jQuery("input[name=softeria_alerts_customer_validation_otp_token]").val();
									if( typeof sa_otp_settings !=  "undefined" && sa_otp_settings["show_countrycode"] == "on" ){
							        ...
							        jQuery("#salert_message").empty(), jQuery("#salert_message").append("' . wp_kses_post($img) . '"), jQuery("#salert_message").show(), jQuery.ajax({
							            url: "' . esc_attr(site_url()) . '/?option=softeria-alert-ajax-otp-validate",
							            type: "POST",
							            data: {softeria_alerts_customer_validation_otp_token: e,billing_phone:f,redirect_to:r},
							            crossDomain: !0,
							            dataType: "json",
							            success: function(o) {
							                if (o.result == "success") {
