...
<div class="cvt-accordion">
    ...
    <?php foreach ( $ninja_forms as $ks => $vs ) { ?>
        <div class="cvt-accordion-body-title" data-href="#accordion_cust_<?php echo esc_attr($ks); ?>">
            <input type="checkbox" name="softeria_alerts_ninja_general[ninja_order_status_<?php echo esc_attr($ks); ?>]" id="softeria_alerts_ninja_general[ninja_order_status_<?php echo esc_attr($ks); ?>]" class="notify_box" <?php echo ( ( softeria_alerts_get_option('ninja_order_status_' . esc_attr($ks), 'softeria_alerts_ninja_general', 'on') === 'on' ) ? "checked='checked'" : '' ); ?>/><label><?php echo esc_attr(ucwords(str_replace('-', ' ', $vs))); ?></label>
        ...
        ?>
                ...
                <tr>
                    <td>
                        Select Phone Field : 
                        <select name="softeria_alerts_ninja_general[ninja_sms_phone_<?php echo esc_attr($ks); ?>]">
        <?php
        $phone_field = trim(softeria_alerts_get_option('ninja_sms_phone_' . $ks, 'softeria_alerts_ninja_general', ''));
        foreach ( $fields as $field ) {
            if (! is_array($field) ) {
                ...
                if ($field!='') {
                    ?>
                                <option value="<?php echo esc_attr($field); ?>" <?php echo ( trim(softeria_alerts_get_option('ninja_sms_phone_' . $ks, 'softeria_alerts_ninja_general', '')) === $field ) ? 'selected="selected"' : ''; ?>><?php echo esc_attr($field); ?></option>
                     <?php
                }
            }
        }
        ?>
                ...
                <tr>
                    <td><input data-parent_id="softeria_alerts_ninja_general[ninja_order_status_<?php echo esc_attr($ks); ?>]" type="checkbox" name="softeria_alerts_ninja_general[ninja_otp_<?php echo esc_attr($ks); ?>]" id="softeria_alerts_ninja_general[ninja_otp_<?php echo esc_attr($ks); ?>]" class="notify_box" <?php echo ( ( softeria_alerts_get_option('ninja_otp_' . esc_attr($ks), 'softeria_alerts_ninja_general', 'off') === 'on' ) ? "checked='checked'" : '' ); ?>/><label>Enable Mobile Verification</label>
                    </td>
                </tr>
            </table>
        </div>
