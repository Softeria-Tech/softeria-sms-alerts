...
<div class="cvt-accordion <?php echo $disablePlayground; ?>">
    ...
    <?php foreach ( $soft-sms-alerts as $ks => $vs ) { ?>
        <div class="cvt-accordion-body-title" data-href="#accordion_<?php echo esc_attr($ks); ?>">
            <input type="checkbox" name="softeria_alerts_fluent_general[fluent_admin_notification_<?php echo esc_attr($ks); ?>]" id="softeria_alerts_fluent_general[fluent_admin_notification_<?php echo esc_attr($ks); ?>]" class="notify_box" <?php echo ( ( softeria_alerts_get_option('fluent_admin_notification_' . $ks, 'softeria_alerts_fluent_general', 'on') === 'on' ) ? "checked='checked'" : '' ); ?>/><label><?php echo esc_html(ucwords(str_replace('-', ' ', $vs))); ?></label>
